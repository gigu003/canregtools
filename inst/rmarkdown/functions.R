...
bbarplot <- function(data, x = year, y = value, fill = sex, facet = type,
                     label = ratio, xlab = "年份", ylab = "发病数/死亡数",
...
bscatter <- function(data, x = year, y = site, fill = asr_cn2000,
                     label = asr_cn2000, facet = ~ sex,
                     xlab = "年份",
                     ylab = "部位",
                     title = "",
                     caption = "") {
  ggplot(data, aes(x = {{x}}, y = {{y}})) +
    ...
    geom_text(aes(label = {{label}}), vjust = 1.5, color = "black", size = 2.5)+
