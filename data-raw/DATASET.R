...
part2 <- all %>% 
  filter(nchar(topo)>4,!is.na(icd10)) %>%
  mutate(topo=gsub("[FM]","",topo),
         topo=gsub("\\[\\]","",topo),
         icd10=gsub("[MF]","",icd10),
         icd10=gsub("\\[\\]|\\(\\)","",icd10)) %>% 
  separate_wider_delim(topo,delim = ",",names = c("topo1","topo2")) %>% 
  separate_wider_delim(icd10,delim = ",",names = c("icd10_1","icd10_2"),too_few = "align_start") 
