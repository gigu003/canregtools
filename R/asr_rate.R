...
asr_rate <- function(
    ...
    show.var = FALSE) {
  groups <- quos(...)
  cr <- function(x, pop) { sum(x) / sum(pop) }
  crvar <- function(x, pop) { sum(x) / sum(pop)^2 }
  asr <- function(x, event, pop) { sum(x / sum(x) * (event / pop)) }
  ...
  crlower <- function(x, pop) {
    if (method == "gamma") {
      ...
      exp(log(cr(x,pop)) + qnorm((1-sig)/2)*sqrt(crvar(x, pop))/cr(x,pop))
    }
  }
  ...
  asrlower <- function(x, event, pop) {
    if (method == "gamma") {
      qgamma((1-sig)/2, shape = asr(x, event, pop)^2/asrvar(x,event,pop))/
