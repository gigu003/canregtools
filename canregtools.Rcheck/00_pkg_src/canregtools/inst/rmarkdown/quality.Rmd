...
asr1 |>
  ...
  drop_total() |>
  add_labels(label_type = "abbr") |>
  select(year, cancer_cn, sex, asr_cn2000) |>
...
agerate |>
  ...
  draw_linechart(x = agegrp, y = rate, group = sex, grid = c(5, 2),
                 ...
                 facet = cancer_cn, axis_title = c("年龄", "发病率(1/10万)"),main=cancer_cn)
```
...
qua2 |>
  filter(!sex == "合计") |>
  bscatter(x = factor(year), y = cancer_cn, fill = mv, label = mv, facet = ~ sex,
